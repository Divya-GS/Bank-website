...
def ViewCustomer():
    ...
    if request.method=="GET":
        out=f.get_customers()
        return render_template('customers.html',out=out,count=1)
...
def Check_Transfer():
    ...
    if request.method=="POST":
        ...
        if res==1:
            flash("Amount Transfered")
            out=f.get_customers()
            return render_template('customers.html',out=out,count=1,flag=1)
        else:
            flash("Please check Receiver details")
            out=f.get_customers()
            return render_template('customers.html',out=out,count=1,flag=0)
